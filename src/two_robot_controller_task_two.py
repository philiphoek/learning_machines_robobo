...
class robotController(Controller):
	def __init__(self, rob):
		...
		self.top_left_two = 0
		self.top_center_two = 0
		self.top_right_two = 0
		self.bottom_left_two = 0
		self.bottom_center_two = 0
		self.bottom_right_two = 0

	def makeStep(self, controller: np.array):
		left, right = self.control(controller)
		left_two, right_two = self.control_two(controller)
		self.rob.move(left, right, left_two, right_two, 1000)
	...
	def control_two(self, controller: np.array):
		inputs = self.getInputValues_two()
		if self.n_hidden[0]>0:
			# Preparing the weights and biases from the controller of layer 1
	...
	def getInputValues(self):
		values = np.array([
			*self.get_irs_values_two(),
			*self.detect_green(self.rob.get_image_front()),
		], float)

		return np.nan_to_num(values)

	def getInputValues_two(self):
		values = np.array([
			*self.get_irs_values_two(),
			*self.detect_green_two(self.rob.get_image_front_two()),
		], float)
	...
	def get_irs_values_two(self):
		irs_values = self.rob.read_irs_two()
		self.front_C_two = irs_values[0]
		self.back_C_two = irs_values[1]
		self.front_LL_two = irs_values[2]
		self.front_RR_two = irs_values[3]
		self.back_L_two = irs_values[4]
		self.back_R_two = irs_values[5]
		self.front_R_two = irs_values[6]
		self.front_L_two = irs_values[7]
		...
		# print(self.back_R)
		return [
			max(self.front_RR_two, self.front_R_two),
			max(self.front_LL_two, self.front_L_two),
			self.front_C_two,
			self.back_C_two,
			self.back_L_two,
			self.back_R_two
	...
	def detect_green_two(self, image):

		cv2.imwrite("test_pictures_two.png", image)
		# resize if image is not the correct resolution
		if image.shape[0] != 128 or image.shape[1] != 128:
			im = Image.open("test_pictures_two.png")
			size = 128, 128
			im_resized = im.resize(size, Image.ANTIALIAS)
			im_resized.save("image-128-128_two.png", "PNG")
			image = cv2.imread("image-128-128_two.png", cv2.IMREAD_UNCHANGED)
		...
		cv2.imwrite("image-128-128_two.png", image)
		...
		if 42 < best_x < 84 and best_y <= 64:
			# print('object is in top center')
			self.top_center_two = 1
		if best_x > 84 and best_y <= 64:
			# print('object is in top right')
			self.top_right_two = 1
		if best_x < 42 and best_y > 64:
			# print('object is in bottom left')
			self.bottom_left_two = 1
		if 42 < best_x < 84 and best_y > 64:
			# print('object is in bottom center')
			self.bottom_center_two = 1
		if best_x > 84 and best_y > 64:
			# print('object is in bottom right')
			self.bottom_right_two = 1

		return [
			max(self.top_left_two, self.bottom_left_two),
			max(self.top_center_two, self.bottom_center_two),
			max(self.top_right_two, self.bottom_right_two),
		]
	...
	def detect_object_two(self):
		for i in range(8):
			raw_value = np.nan_to_num(self.rob.read_irs_two())[i]
			if raw_value != 0:
				value = np.log(np.array(raw_value))
				if (value / 10 > -1) and (value / 10 < -0.20):
					return True
